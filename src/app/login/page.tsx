  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (mode === "register") {
      if (!name.trim() || !email.trim() || password.length < 6) {
        setError("املأ كل البيانات (كلمة السر 6 أحرف على الأقل)");
        return;
      }
      ...
      localStorage.setItem("arlo-user", JSON.stringify({ name, email, isAdmin: false }));
    ...
    <div className="min-h-[80vh] flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-md bg-white rounded-2xl shadow-xl border border-[#e8d5b7]/40 p-8">
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-gray-500 mt-1">Arlo Wood</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {mode === "register" && (
            <div>
              <label className="block text-sm font-medium mb-1 text-[#3d2b1f]">الاسم</label>
              <input
                type="text"
                value={name}
          ...
          <div>
            <label className="block text-sm font-medium mb-1 text-[#3d2b1f]">الإيميل</label>
            <input
              type="email"
              value={email}
          ...
          <div>
            <label className="block text-sm font-medium mb-1 text-[#3d2b1f]">كلمة السر</label>
            <input
              type="password"
              value={password}
